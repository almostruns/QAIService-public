...
[[nodiscard]] std::string_view moduleName(Module module);
...
class LogStream {
 public:
  LogStream(spdlog::source_loc location, spdlog::level::level_enum level, Module module)
  ...
  ~LogStream()
  {
    spdlog::default_logger_raw()->log(location_, level_, "[{}] {}", moduleName(module_), buffer_.str());
  }
...
// 统一日志入口：QAI_LOG(info, qaiservice::log::Module::kChat) << "message key=value";
